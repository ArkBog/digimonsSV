...
// @ts-nocheck
  ...
  const filters = () => {
    ...
    for(let i = 0; i < $digimons.length; i++){
      if(filtersArray.includes($digimons[i].level) && filtersArray.length != 0){
        continue
      }
      else{
  ...
  let runFilter = (...arg)=>{
    console.log(arg)
    return digimonsToDisplay =  digimonsToDisplay.filter((e)=>arg.includes(e.level)) ;
  }
...
<div class="container">
  ...
  <div class="filters">
    {#each filters() as filter }
      <input type="checkbox" name="filter" value={filter} on:change={(event)=>runFilter(event.target.value)}>
    ...
    {#each digimonsToDisplay as digimon}
      <div class="card" >
        ...
        <div class="card-img">
          <img class="digimon-img" src={digimon.img} alt={digimon.name} on:click={() => navigateToComponent()}/>
          <button class="add-to-fav" on:click={() => addToFavourites(digimon)}>Add to favourites</button>
        </div>
